...
//	};
	...
	public byte[] sm2GetZ(byte[] userId, ECPoint userKey)
	{
		...
		byte[] p = SMTypeCastUtil.byteConvert32Bytes(ecc_a);
		sm3.update(p, 0, p.length);
		
		p = SMTypeCastUtil.byteConvert32Bytes(ecc_b);
		sm3.update(p, 0, p.length);
		
		p = SMTypeCastUtil.byteConvert32Bytes(ecc_gx);
		sm3.update(p, 0, p.length);
		
		p = SMTypeCastUtil.byteConvert32Bytes(ecc_gy);
		sm3.update(p, 0, p.length);
		
		p = SMTypeCastUtil.byteConvert32Bytes(userKey.getXCoord().toBigInteger());
		sm3.update(p, 0, p.length);
		
		p = SMTypeCastUtil.byteConvert32Bytes(userKey.getYCoord().toBigInteger());
		sm3.update(p, 0, p.length);
